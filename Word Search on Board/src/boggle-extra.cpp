/*
 * File: boggle-extra.cpp
 * -----------------------
 * Assignment 4 Part C Coding Boggle
 *
 * Extra feature 1: Embellish the GUI. Sound effects are added.
 *
 * Extra feature 2: big boggle. This program can generate bigger board with size of 5x5.
 *
 * Extra feature 3: Make the Q a useful letter. In this case, Boggle will regard Q as QU. If user's input
 * word contains Q, he/she must type a U following that Q. During the search, the program will take Q on
 * board as QU both for human player and computer player.
 *
...
/* Mian program */
int main() {
    gui::initialize(BOARD_SIZE+1, BOARD_SIZE+1);
    gui::playSound("come-on-faster.wav");
    intro();
    menu();
...
void intro() {
    ...
    cout << "If only YOU had 25 gigs of RAM!" << endl;
    cout << "You must use Q together with u!" << endl;
    cout << endl;
}
...
void menu() {
    ...
    if(startChoice.empty()) {
        ...
        do {
            if(playAgain) {
                gui::initialize(BOARD_SIZE+1, BOARD_SIZE+1); // clear the board on GUI if user plays again
                gui::playSound("come-on-faster.wav");
            }
            cout << endl;
            Grid<char> board(BOARD_SIZE+1, BOARD_SIZE+1);
            ...
            if(humanScore < computerScore) {
                ...
                gui::playSound("thats-pathetic.wav");
            } else {
                cout << "WOW, you defeated me! Congratulations!" << endl << endl;
                gui::playSound("excellent.wav");
            }
...
void generateBoard(Grid<char>& board) {
    ...
    if(getYesOrNo("Generate a random board?")) {
        // option1
        for(int i = 0; i < LETTER_CUBES_BIG.size(); i++) {
            string letters = LETTER_CUBES_BIG[i];
        ...
        while(true) {
            boardString = getLine("Type the 25 letters on the board:");
            boardString = toUpperCase(boardString);
            if(boardString.length() == (BOARD_SIZE+1)*(BOARD_SIZE+1) && allLetters(boardString)) {
                break; // exit the loop if the string's length is 16, only containing letters
            }
            gui::playSound("moo.wav");
    ...
    for(size_t i = 0; i < boardString.length(); i++) {
        board[int(i)/(BOARD_SIZE+1)][int(i)%(BOARD_SIZE+1)] = boardString[i];
        cout << boardString[i];
...
bool humanWordSearch(Grid<char>& board, string word) {
    ...
    Grid<bool> isVisited((BOARD_SIZE+1), (BOARD_SIZE+1), false);
...
bool DFS1(Grid<char>& board, Grid<bool>& isVisited, int i, int j, string word) {
    ...
    if(board[i][j] == word[0]) {
        // extra feature: assume that user type Q and U together, even tough U is not on board
