/*
 * File: grammarsolver-extra.cpp
 * -----------------------
 * Assignment 3 Part C Coding Grammar Solver
 *
 * Extra feature is robust grammar solver. This grammar solver is able to handle files with excessive
 * whitespace placed between tokens.
 *
...
Map<string, Vector<string>> readInputExtra(istream& input);
 ...
 * This function will call helperfunction of readInputExtra to build a map which records the grammar in the
...
Vector<string> grammarGenerate(istream& input, string symbol, int times) {
    ...
    Map<string, Vector<string>> map = readInputExtra(input);
    Vector<string> v;
    repeatedGenerate(symbol, times, map, v);
    return v;
}
...
Map<string, Vector<string>> readInputExtra(istream& input) {
    ...
    while(getline(input, line)) {
        line = trim(line);
        Vector<string> lineVec = stringSplit(line, "::=");
        string name = trim(lineVec[0]); // remove outer whitespaces in the line
        ...
        if(map.containsKey(name)) {
            ...
            Vector<string> rules = stringSplit(trim(lineVec[1]), "|"); // a vector of terminals or non-terminals
            for(int i = 0; i < rules.size(); i++) {
                rules[i] = trim(rules[i]); // remove outer whitespaces for each rule
                Vector<string> tokens = stringSplit(rules[i], " ");
                // case of tokens.size() == 1 is only one token and no whitespace
                if(tokens.size() > 1) {
                     string tokensTrim;
                     for(string token : tokens) {
                         // whitespace between tokens will be split into empty string, ""
                         if(token != "") {
                             tokensTrim += (token+" ");
                         }
                     }
                     tokensTrim = trim(tokensTrim); // remove the whitespace at the end
                     rules[i] = tokensTrim; // update rules by trimmed tokens
                }
            }
            map[name] = rules;
        }
