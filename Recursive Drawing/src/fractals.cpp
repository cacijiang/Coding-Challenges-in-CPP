...
void floodHelper(GWindow& window, int x, int y, int preColor, int color, int& counter);
...
int floodFill(GWindow& window, int x, int y, int color) {
    ...
    floodHelper(window, x, y, preColor, color, counter);
    return counter;
}

/*
 * This is a helper recursive function to draw triangles.
...
void triangleHelper(GWindow& window, double x, double y, double size, int order, int n) {
    ...
    if(n == 1) {
        window.drawPolarLine(x, y, size, 0); // horizontal edge
        window.drawPolarLine(x+size, y, size, 240); // inclined edge
        window.drawPolarLine(x, y, size, -60); // inclined edge
        double newX = x+size/2; // for next time start from the midpoint of the horizontal edge
        double newSize = size/2; // update size
        ...
        double xLeft = x-size/2; // update x for next left triangle
        window.drawPolarLine(x, y, size, -60);
        double xRight = x+size/2; // update x for next right triangle
        double yDown = y+size*sqrt(3)/2; // update y for next bottom triangle
 ...
 * Base case 2: current pixel has been filled by desired color or is the same as desired color. Or current
 *              pixel is not the same as original color, which means (x, y) goes outside the rectangles
 *              that user wants to fill.
 * Recursive case: change the color of current pixel. Update counter. Explore and continue filling in four
...
void floodHelper(GWindow& window, int x, int y, int preColor, int color, int& counter) {
    ...
    // base case 2
    if(window.getPixel(x, y) == color || window.getPixel(x, y) != preColor) {
        return;
    }
    // recursive case
    window.setPixel(x, y, color);
    counter++;
    floodHelper(window, x, y-1, preColor, color, counter);
    floodHelper(window, x+1, y, preColor, color, counter);
    floodHelper(window, x, y+1, preColor, color, counter);
    floodHelper(window, x-1, y, preColor, color, counter);
}
